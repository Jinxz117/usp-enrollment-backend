...
//   } catch (error) {
//     console.error("Error fetching eligible courses:", error);
//     res.status(500).json({ error: "Failed to fetch eligible courses",code: "ENROLLMENT_ERROR" });
//   }
// };
...
const EnrollmentModel = require("../Model/enrollmentModel");
const db = require("../Model/db"); // Use the updated db.js
...
exports.getEligibleCoursesForProgramRe = async (req, res) => {
  ...
  try {
    ...
    console.error("Error fetching eligible courses:", error);
    res.status(500).json({ error: "Failed to fetch eligible courses", code: "GET_ELIGIBLE_COURSES_ERROR" });
  }
};
...
exports.enrollStudent = async (req, res) => {
  ...
  if (!student_id || !course_id) {
    return res.status(400).json({ error: "Student ID and Course ID are required", code: "MISSING_FIELDS" });
  }
  ...
  try {
    ...
    console.error("Error enrolling student:", error);
    res.status(500).json({ message: "Failed to enroll student", code: "ENROLL_STUDENT_ERROR" });
  }
};
...
exports.updateEnrollmentStatus = async (req, res) => {
  ...
  if (!enrollment_id || !status) {
    return res.status(400).json({ error: "Enrollment ID and status are required", code: "MISSING_FIELDS" });
  }
  ...
  try {
    const affectedRows = await EnrollmentModel.updateEnrollmentStatus(enrollment_id, status);
    if (affectedRows === 0) {
      return res.status(404).json({ message: "Enrollment not found", code: "ENROLLMENT_NOT_FOUND" });
    }
    res.status(200).json({ message: "Enrollment status updated successfully" });
  } catch (error) {
    console.error("Error updating enrollment status:", error);
    res.status(500).json({ message: "Failed to update enrollment status", code: "UPDATE_ENROLLMENT_ERROR" });
  }
};
...
exports.getStudentEnrollments = async (req, res) => {
  ...
  if (!student_id) {
    return res.status(400).json({ error: "Student ID is required", code: "MISSING_FIELDS" });
  }
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ message: "No enrollments found for this student", code: "ENROLLMENTS_NOT_FOUND" });
    }
    res.status(200).json(rows); // Return enrollments with course details
  } catch (error) {
    console.error("Error fetching enrollments:", error);
    res.status(500).json({ message: "Server error", code: "GET_ENROLLMENTS_ERROR" });
  }
};
...
exports.deleteEnrollment = async (req, res) => {
  ...
  if (!enrollmentId) {
    return res.status(400).json({ error: "Enrollment ID is required", code: "MISSING_FIELDS" });
  }
  ...
  try {
    ...
    console.error("Error deleting enrollment:", error);
    res.status(500).json({ message: "Failed to delete enrollment", code: "DELETE_ENROLLMENT_ERROR" });
  }
};
